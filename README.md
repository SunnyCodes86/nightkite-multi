...
`SyncEngine` und `PatternClock` verwalten in diesem Alpha-Schritt lokale Sync-Zustaende, geplante lokale Starts, Pattern, Helligkeit, Phase und Pattern-Zeit. `sync_arm`, `sync_status` und `sync_cancel` funktionieren lokal und loesen keine direkten EEPROM-Schreibvorgaenge aus; automatische Config-Saves werden waehrend aktivem lokalen Sync-Timing zurueckgestellt. Das experimentelle Sync-Beacon-V1-Modell uebertraegt Group, Flags, Sequenz, Pattern, Helligkeit, Phase, Beat und CRC. Follower akzeptieren zusaetzlich V2-Beacons mit Energy, Bass, Mid, Treble und Confidence. Die V2-Flags sind `BEAT=0x01`, `SIGNAL_VALID=0x02` und `BEAT_LOCKED=0x04`; Paketgroesse und CRC-Layout bleiben unveraendert. Die V2-Audiodaten werden als kurzlebiger Runtime-Status gespeichert und steuern die Patterns 23 bis 27; die Audioanalyse selbst findet im Cardputer statt. Es werden keine LED-Frames ueber Funk gestreamt.
...
Die neuen Audio-Patterns sind `audio_pulse_angle_color` (23), `audio_spectrum_ribbon` (24), `audio_beat_ripples` (25), `audio_band_comets` (26) und `audio_beat_mosaic` (27). Energy und Frequenzbaender bestimmen Helligkeit, Form und Akzente; Beat, BeatMs und Phase halten die Bewegung synchron. Yaw, Pitch oder lokale Bewegung beeinflussen weiterhin die Farbe. Bei V1, `SIGNAL_VALID=0` oder wenn laenger als 500 ms kein neues gueltiges V2-Audioframe empfangen wurde, werden alle fuenf Patterns schwarz und ihre Audiohistorie wird zurueckgesetzt. Bei gueltigem Signal ohne `BEAT_LOCKED` bleiben Energy und Baender aktiv, Phase und synthetischer Beat-Puls aber null. Es gibt keinen lokalen oder synthetischen Audiofallback. Werkseinstellungen aktivieren alle 27 Patterns. Config-Version 401 aktiviert die neuen IDs einmalig auch in aelteren gespeicherten Pattern-Masks; Config-Version 402 ergaenzt den persistenten Show-Control-Empfang, der bei Migration aus 400/401 standardmaessig aus bleibt.
...
Der Controller-Master sendet weiterhin unveraendert V1. Der Follower-Decoder akzeptiert V1 und V2; beide Versionen speisen dieselben bestehenden Sync-Basisdaten ein. V2 ist 22 Byte gross, hat 24 Byte Manufacturer Data inklusive Company ID und passt damit in ein 29-Byte-Legacy-Advertising-Paket. Die Manufacturer-Data-Signatur beginnt mit `FFFF4E4B02...`. Ein frisches V2 setzt `audio_valid` nur mit `SIGNAL_VALID`; alte Sender ohne dieses Bit gelten konservativ als ohne gueltiges Audio. Nur `BEAT_LOCKED` aktiviert BeatMs und Phase. Nach 500 ms ohne neues gueltiges V2-Audioframe wird `audio_valid=0`; identische wiederholte V2-Sequenzen verlaengern die Audio-Freshness nicht. Der allgemeine Controller-Sync-Verlust bleibt davon getrennt bei 1500 ms.
...
Erwartet werden bei echtem Signal `signal_valid=1`, `audio_valid=1`, `last_beacon_version=2`, steigendes `scan_decode_v2`, `sync_locked=1`, `scan_crc_fail=0` und sichtbare Reaktionen auf Pegel und Baender. Beatphase ist erst bei `beat_locked=1` aktiv. Bei geschlossenem Gate bleiben 23 bis 27 trotz frischer V2-Pakete schwarz. Der Test erfordert einen Cardputer/NightKite-Link-Build mit V2-Audio-Sync-Unterstuetzung.
...
Die bestehenden Radio-Diagnosefelder bleiben in `NK4 cmd=get section=sync`, `NK4 cmd=sync_status`, `NK4 cmd=sync_radio_status` und `NK4 cmd=get section=wireless` sichtbar. Die kurze, auch fuer BLE-GATT geeignete Abfrage `NK4 seq=<n> cmd=audio_sync_status` liefert `audio_sync`, `audio_valid`, `signal_valid`, `beat_locked`, `last_beacon_version`, `scan_decode_v1`, `scan_decode_v2`, `audio_seq`, `audio_age_ms`, `audio_beat`, `audio_energy`, `audio_bass`, `audio_mid`, `audio_treble`, `audio_confidence`, `audio_phase_ms`, `audio_beat_ms` und `audio_timeout_ms`.
