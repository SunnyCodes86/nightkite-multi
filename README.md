...
GATT/NK4 bleibt der Konfigurations- und Diagnosepfad. Der Beacon-Radio nutzt keine GATT-Kommandos fuer Echtzeit-Sync und sendet keine Acknowledgements. Wenn ein BLE-GATT-Client verbunden ist, hat der Config/GATT-Modus Vorrang und der Beacon-Radio meldet `radio_mode=gatt`; USB bleibt immer als Fallback verfuegbar. Diagnosefelder sind in `NK4 cmd=get section=sync`, `NK4 cmd=sync_status`, `NK4 cmd=sync_radio_status` und `NK4 cmd=get section=wireless` sichtbar, darunter `sync_radio`, `sync_radio_active`, `beacon_tx`, `beacon_rx`, `codec_selftest`, `scan_active`, `adv_active`, `adv_payload_len`, `adv_mfg_len`, `adv_company`, `adv_magic`, `adv_version`, `adv_sig`, `adv_set_count`, `beacon_seq`, `beacon_tx_count`, `beacon_rx_count`, `beacon_crc_errors`, `beacon_group_mismatch`, `scan_reports`, `scan_mfg_reports`, `scan_nk_candidates`, `scan_decode_ok`, `scan_decode_fail`, `scan_reject_company`, `scan_reject_magic`, `scan_reject_len`, `scan_reject_version`, `scan_last_company`, `scan_last_mfg_head`, `scan_last_candidate_reason`, `scan_last_error`, `last_beacon_ms`, `beacon_age_ms`, `sync_locked`, `radio_mode` und `radio_last_error`.
