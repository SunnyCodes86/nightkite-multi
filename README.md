...
Sync-Timing ist ueber NK4 diagnostizierbar. `sync_status` und `get section=sync` melden u. a. `last_beacon_pattern`, `last_beacon_seq`, `last_applied_seq`, `phase_ms`, `beacon_phase_ms`, `sync_apply_count`, `sync_apply_skipped`, `sync_apply_reason`, `pattern_change_count`, `last_pattern_change_latency_ms` und `sync_ready_pattern`. `patterns` und `get section=patterns` enthalten zusaetzlich `sync_ready_mask` und `local_reactive_mask`. In diesem Alpha-Schritt sind die zeitbasierten Patterns 1 `rainbow`, 4 `runner_fixed` und 7 `heartbeat` als erste sync-ready Kandidaten markiert und nutzen PatternClock fuer ihre Animationsphase. Motion-/MPU-reactive Patterns bleiben lokal-reactive; sie koennen Pattern und Helligkeit folgen, aber ihre visuelle Bewegung darf sich je Controller unterscheiden.
...
Diagnosefelder sind in `NK4 cmd=get section=sync`, `NK4 cmd=sync_status`, `NK4 cmd=sync_radio_status` und `NK4 cmd=get section=wireless` sichtbar, darunter `sync_radio`, `sync_radio_active`, `beacon_tx`, `beacon_rx`, `codec_selftest`, `scan_active`, `adv_active`, `adv_owner`, `adv_type`, `gatt_adv_suppressed`, `beacon_adv_started`, `beacon_adv_refreshes`, `adv_enable_count`, `adv_disable_count`, `adv_payload_len`, `adv_mfg_len`, `adv_company`, `adv_magic`, `adv_version`, `adv_sig`, `adv_set_count`, `beacon_seq`, `beacon_tx_count`, `beacon_rx_count`, `beacon_crc_errors`, `beacon_group_mismatch`, `scan_reports`, `scan_mfg_reports`, `scan_nk_candidates`, `scan_decode_ok`, `scan_decode_fail`, `scan_reject_company`, `scan_reject_magic`, `scan_reject_len`, `scan_reject_version`, `scan_last_company`, `scan_last_mfg_head`, `scan_last_candidate_reason`, `scan_last_error`, `last_beacon_ms`, `beacon_age_ms`, `sync_locked`, `radio_mode` und `radio_last_error`.
